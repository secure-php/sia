...
add_to_hash:
	...
	/*
	* SymPHP
	* check if a value is stored to super global, and accordingly set symbolic
	*/
	    ...
	    ht == Z_ARRVAL(PG(http_globals)[TRACK_VARS_POST])) {
			if(! s2e_is_symbolic(&p->val, sizeof(p->val))) {
				...
				if (sym_name != NULL) {
					...
					// zend_error(E_WARNING,  "SymPHP: now make it symbolic. val %s: size %ld, sizeof %ld before symbolic",p->val.value.str->val, p->val.value.str->len, sizeof(p->val.value.str->val));
